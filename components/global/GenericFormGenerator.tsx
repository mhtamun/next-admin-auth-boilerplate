...
import { Input, FileInput, SelectSync, SelectAsync, RadioGroup, CheckboxGroup } from '../index';
...
export default function GenericFormGenerator({
    ...
    function getField(field) {
        const errorMessage = !formik.touched[field.name]
            ? ''
                ...
                <Input
                    ...
                    onChange={(e) => {
                        ...
                        if (field.type === 'number') {
                            if (field.onChange)
                                field.onChange(parseFloat(e.target.value), formik.values, formik.setFieldValue);
                        } else {
                            if (field.onChange) field.onChange(e.target.value, formik.values, formik.setFieldValue);
                        }
                    }}
    ...
    return (
        <></>
        // <CForm onSubmit={formik.handleSubmit} onKeyDown={onKeyDown}>
        //     {formFields.map((tempField) => tempField)}
        //     {submitButton}
        //     {!resetButtonText ? null : (
        //         <CButton
        //             className="mt-1 ml-3"
